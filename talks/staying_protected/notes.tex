...
\usepackage{fixltx2e} % provides \textsubscript1
...
\subsubsection{``libel'' and ``slander''}

Defamation is an umbrella term for two distinct kinds of claim (technically known as \textbf{causes of action}), which are:
...
``application'' at any
time.\footnote{Readers familiar with pre-6th edition
\emph{Magic: the Gathering} will find applications reminiscent of
interrupts.} Applications are very flexible. In rare circumstances:
...
\subsubsection{natural and ordinary}
...
\begin{itemize}
\item   natural and ordinary meaning includes imputations and inferences
  \begin{itemize}
  \item
    ``Have you heard that Fox was reported twice as a spy'' -
    defamatory implication that he was
    guilty.\footnote{\emph{Fox v Goodfellow} (1926) NZLR 58}
  \end{itemize}
\item
...
\item
  ...
  \item
    ...
    and tomato sauce'''\footnote{Lord Justice Scrutton in \emph{Bennison v
...
In {\it Lewis v Daily Telegraph Ltd}\footnote{\[1964\] A.C. 234} Lord Devlin observed\footnote{at p.283--4 } that:
...
suspicion.\footnote{Rubber Improvement Ltd v Daily
Telegraph}{ {[}1964{]} A.C. 234}

The courts have developed what are called the three \emph{Chase} levels of meaning:\footnote{\emph{Chase v News Group Newspapers}
{[}2003{]} EMLR 11}
...
\begin{itemize}
\item
  ``the man who lives in that house is a paedophile''\footnote{Channel 7 Sydney v Parras {[}2002{]} NSWCA}
\item
  ``X is illegitimate''\footnote{Cassidy v Daily Mirror}
\item
...
\item
  ...
  \begin{itemize}
  \item
    ``all lawyers are thieves'' would not permit any lawyer to sue\footnote{Eastwood v Holmes}
  \item  Knupffer v London Express Newspaper was a case where the class of people (members of an organisation) was very small, but the claim still failed for lack of proof that a particular individual was intended.
  \end{itemize}
\end{itemize}
...
well\footnote{Goldsmith v Bhoyrul {[}1998{]} Q.B. 459} but we do not have
...
corporations\footnote{MacDonald's Corporation v Steel (No. 4),
the Independent, May 10, 1999} although if
they are unable to show any trading loss their damages will be kept in
``tight bounds''.\footnote{Jameel v Wall Street Journal Europe SPRL
...
\appendix\label{sec:applibelvslander}
...
\subsection{Capital and Counties Bank v George Henty \& Sons}

% \footnote{(1881-82) L.R. 7 App. Cas. 741}
...
\subsection{Hulton v Jones}

% \footnote{{[}1910{]} A.C. 20
...
{Fox's Libel Act\footnote{Libel Act 1792 (32 Geo. III c. 60)} reformed
